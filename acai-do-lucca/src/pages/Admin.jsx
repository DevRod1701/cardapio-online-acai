...
import { 
  Plus, Trash2, ArrowLeft, Edit3, Search, X, Check, 
  LayoutGrid, List, Package, Loader2, ArrowUp, ArrowDown, Upload, Layers, LogOut, Users, Phone, MapPin
} from 'lucide-react';
import { useData } from '../hooks/useData';

export default function Admin({ onBack }) {
  // 1. Adicionei 'logout' aqui na desestruturação
  ...
  const handleLogout = async () => {
      // 1. Faz logout no Supabase (limpa token e storage)
      await logout();
      // 2. Só depois volta para a tela inicial
      onBack();
  };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Excluir permanentemente?")) await deleteData(activeTab, id);
  };
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gray-50 font-sans text-gray-800 overflow-hidden">
      ...
      <aside className="w-full md:w-64 bg-white border-b md:border-r border-gray-200 flex flex-row md:flex-col shrink-0 z-20 h-auto md:h-full">
         <div className="p-4 md:p-6 flex items-center justify-between md:justify-start gap-3 w-full md:w-auto">
            ...
            {/* SAIR (MOBILE) USANDO handleLogout */}
         ...
         <nav className="hidden md:flex flex-1 px-4 flex-col space-y-2 mt-4">
         ...
         <div className="hidden md:block p-4 mt-auto">
            {/* SAIR (DESKTOP) USANDO handleLogout */}
